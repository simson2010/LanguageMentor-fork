...
class VocabManager(ABC):

    def __init__(self, agent = vocab_agent, feature=feature, title="单词", subtitle="闯关背单词"):
        self.agent = agent 
    ...
    def get_page_desc(self, feature):
        try:
    ...
    # 重新启动词汇学习聊天机器人会话
    def restart_vocab_study_chatbot(self):
        self.agent.restart_session()  # 重启会话

        # 定义初始消息并与词汇代理交互生成机器人的回应
        _next_round = "Let's do it"
        bot_message = self.agent.chat_with_history(_next_round)
    ...
    def handle_vocab(self, user_input, chat_history):
        bot_message = self.agent.chat_with_history(user_input)  # 获取机器回复
    ...
    def create_vocab_tab(self):
        ...
        with gr.Tab(self.title):
            gr.Markdown(f"## {self.subtitle}")  # 添加 Markdown 标题

            # 显示从文件中获取的页面描述
            gr.Markdown(self.get_page_desc(feature))
